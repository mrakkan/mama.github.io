...
#### ! ####
    ...
    return render_template('home.html',sum_food=sum_food, sum_education=sum_education, sum_travelfare=sum_travelfare, \
...
def main():
    financelist = Finance.query.filter_by(user_id=current_user.id).first() 
    balance = (financelist).balance
...
def add():
    ...
    todo = Todo.query.filter_by(user_id=current_user.id, text=request.form['todoitem'], complete=False) # !
    db.session.add(todo)# !
    db.session.commit()
    return redirect(url_for('views.todoshow'))
...
#### ! ####
    finun = render_template('finance.html',sum_food=sum_food, sum_education=sum_education, sum_travelfare=sum_travelfare, \
                            sum_goodsservices=sum_goodsservices, sum_other=sum_other,
                            sumincome=sumincome, sumoutcome=sumoutcome, sumsavings=sumsavings, \
                            message=message, balance=balance, history10=history10)
